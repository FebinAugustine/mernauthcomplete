...
const AddUser = () => {
  ...
  useEffect(() => {
    const fetchFellowships = async () => {
      try {
        const { getAllFellowships } = await import("../../api/admin.api");
        const data = await getAllFellowships();
        // Filter fellowships by user's subzone
        const filtered = (data.fellowships || []).filter(
          (f) => f.subzone?._id === user?.subZone?._id
        );
  ...
  const fetchUsers = async (page = 1, searchParam = search) => {
    ...
    try {
      ...
      // Filter users to only those in the user's subzone
      const filteredUsers = (data.users || []).filter(
        (u) => u.subZone?._id === user?.subZone?._id
      );
