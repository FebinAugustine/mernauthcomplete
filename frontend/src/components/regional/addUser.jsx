...
const AddUser = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [fellowshipsData, subzonesData, zonesData] = await Promise.all([
          getAllFellowships(),
          getAllSubzones(),
          getAllZones(),
        ]);
        // Filter data to only those in the user's region
        const filteredFellowships = (fellowshipsData.fellowships || []).filter(
          (fellowship) => fellowship.region?._id === user?.region?._id
        );
  ...
  const fetchUsers = async (page = 1, searchParam = search) => {
    ...
    try {
      ...
      // Filter users to only those in the user's region
      const filteredUsers = (data.users || []).filter(
        (u) => u.region?._id === user?.region?._id
      );
