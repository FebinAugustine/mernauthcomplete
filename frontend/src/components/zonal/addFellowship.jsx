...
const AddFellowship = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [fellowshipsData, subzonesData] = await Promise.all([
          getAllFellowships(),
          getAllSubzones(),
        ]);
        // Filter fellowships to only those in the user's zone
        const filteredFellowships = (fellowshipsData.fellowships || []).filter(
          (fellowship) => fellowship.zone?._id === user?.zone?._id
        );
        // Filter subzones to only those in the user's zone
        const filteredSubzones = (subzonesData.subZones || []).filter(
          (subzone) => subzone.zone?._id === user?.zone?._id
        );
              ...
              >
                ...
                <div className="space-y-3">
                  <div className="flex items-center text-sm text-gray-600">
                    ...
                    <span>{fellowship.subzone?.name || "N/A"}</span>
                  </div>
                  <div className="flex items-center text-sm text-gray-600">
